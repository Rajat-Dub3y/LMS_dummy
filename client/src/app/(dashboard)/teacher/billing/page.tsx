...
const TeacherBilling = () => {
    ...
    const {data:transactions,isLoading:isLoadingTransaction}=useGetTransactionsQuery(user?.id || "",{skip:!isLoaded || !user});
            ...
            <div className="billing__filters">
                <Select value={paymentType} onValueChange={setPaymentType} >
                    <SelectTrigger className="billing__select" >
                        <SelectValue placeholder="Paymnet Type" />
                    </SelectTrigger>
                        ...
                        <TableBody className='billing__table-body' >
                            {filteredData.length>0 ? (
                                filteredData.map((transaction)=>(
                                    <TableRow className='billing__table-row' key={transaction.dateTime}>
                                ...
                                <TableRow className='billing__table-row'>
                                    <TableCell className='billing__table-cell text-center' colSpan={3} >No transactions to show</TableCell>
                                </TableRow>
                            )}
                        </TableBody>
                    </Table>
